            ...
            <div className="relative flex-1 flex flex-col">
                <header className="relative">
                    <div className="relative mx-4 md:ml-14 flex justify-between items-center py-4 md:py-6">
                        ...
                        <button><Link to={"/"} className="mr-4 md:mr-20 bg-[#8E3BEA] text-xs sm:text-sm md:text-base font-semibold rounded-md px-3 md:px-5 py-2 text-white">Sign In</Link></button>
                    </div>
                </header>
                ...
                <div className="flex-1 px-4 sm:px-6 py-8 md:py-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-start max-w-6xl mx-auto">
                        ...
                        <div className="text-white/90 order-2 md:order-1 text-center md:text-left">
                            <p className="font-extrabold text-2xl sm:text-3xl md:text-6xl font-sans text-white p-4 leading-tight drop-shadow-lg">
                                Welcome to BulkMail! Let’s Get Started<br />
                            </p>
                            <p className="text-white text-base sm:text-lg md:text-xl font-semibold mt-2 md:mt-4">Grow Your Audience with BulkMail</p>
                            <p className="mt-4 md:mt-10 text-white/90 text-xs sm:text-sm md:text-base font-semibold px-4">Ready to Supercharge Your Emails?</p>
                            <ul className="mt-5 md:mt-6 space-y-2 md:space-y-3 text-white/85 px-4 md:px-0">
                        ...
                        <div className="order-1 md:order-2">
                            <div className="relative bg-black/60 backdrop-blur-md w-full max-w-md md:ml-auto rounded-2xl shadow-2xl border border-white/10 text-center p-5 sm:p-6 md:p-8">
                                <h2 className="font-bold text-xl sm:text-2xl md:text-4xl font-sans text-white mb-3 md:mb-4">Create your account</h2>
                                {error && <p className="text-red-500 font-bold mb-4 px-3 sm:px-4 text-sm">{error}</p>}
                                <div className="flex flex-col gap-3 mt-2">
                                    <input
                                        onChange={handleUIput}
                                        className="p-3 sm:p-4 rounded-lg w-full text-sm sm:text-base bg-black/50 border-2 border-white/20 text-white placeholder:text-white/70 outline-none transition-all duration-200 focus:border-white focus:ring-4 focus:ring-white/20"
                                        placeholder="Email address"
                                    />
                                    <button
                                        onClick={addUser}
                                        disabled={loading}
                                        className="w-full py-3 rounded-lg text-white text-base sm:text-lg font-semibold whitespace-nowrap disabled:opacity-50 transition-all duration-200 shadow-lg hover:shadow-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500"
                                    >
                                        {loading ? "Please wait..." : "Get Started"}
                                    </button>
                                    <p className="text-white/80 text-xs sm:text-sm">Already have an account? <Link to={'/'} className="text-purple-400 hover:underline">Sign In</Link></p>
                                </div>
                            </div>
                        </div>
                    </div>
